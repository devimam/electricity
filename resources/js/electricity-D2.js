...
var demandcharge = 90; ///per kW for LT-D2
...
function loadLtD2Rates() {
  for (let key in lt_d2) {
    document.getElementById(key).innerHTML = lt_d2[key].toFixed(2); ///upto 2 decimal places
  }
}
...
function doubleregisterunitbill(offpkunit, pkunit) {
  let offpkenergycost = offpkunit * lt_d2["offpkrate"];
  let pkenergycost = pkunit * lt_d2["pkrate"];
...
function singleregisterunitbill(srunit) {
  let srenergycost = srunit * lt_d2["srrate"];
