...
var demandcharge = 60; ///per kW for LT-D1
...
function loadLtD1Rates() {
  for (let key in lt_d1) {
    document.getElementById(key).innerHTML = lt_d1[key].toFixed(2); ///upto 2 decimal places
  }
}
...
function doubleregisterunitbill(offpkunit, pkunit) {
  let offpkenergycost = offpkunit * lt_d1["offpkrate"];
  let pkenergycost = pkunit * lt_d1["pkrate"];
...
function singleregisterunitbill(srunit) {
  let srenergycost = srunit * lt_d1["srrate"];
