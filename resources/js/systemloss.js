...
(
    function() {
        ...
        function calculatesloss() {
            ...
            if (isvalidslossimport && slossimport > 0 && isvalidslosspostpaid && slosspostpaid >= 0 && isvalidslossprepaid && slossprepaid >= 0) {
                let totalsold = slosspostpaid + slossprepaid;
                let systemloss = (1 - (totalsold / slossimport)) * 100;

                document.getElementById('slossvalue').innerHTML = systemloss.toFixed(2) + " %";
                document.getElementById('slossprogress').innerHTML = systemloss.toFixed(2) + " %";
                document.getElementById('slossprogress').style.width = systemloss.toFixed(2) + "%";
            } else {
        ...
        modalelm.addEventListener('show.bs.modal', event => {
            ...
            document.getElementById('slossimport').classList.remove('is-invalid', 'is-valid');

            document.getElementById('slosspostpaid').value = "";
            document.getElementById('slosspostpaid').classList.remove('is-invalid', 'is-valid');

            document.getElementById('slossprepaid').value = "";
            document.getElementById('slossprepaid').classList.remove('is-invalid', 'is-valid');
