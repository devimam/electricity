...
var demandcharge = 90; ///per kW for LT-E
...
function loadLtERates() {
  for (let key in lt_e) {
    document.getElementById(key).innerHTML = lt_e[key].toFixed(2); ///upto 2 decimal places
  }
}
...
function doubleregisterunitbill(offpkunit, pkunit) {
  let offpkenergycost = offpkunit * lt_e["offpkrate"];
  let pkenergycost = pkunit * lt_e["pkrate"];
...
function singleregisterunitbill(srunit) {
  let srenergycost = srunit * lt_e["srrate"];
