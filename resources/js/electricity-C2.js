...
function loadLtC2Rates() {
  for (let key in lt_c2) {
    document.getElementById(key).innerHTML = lt_c2[key].toFixed(2); ///upto 2 decimal places
  }
}
...
function doubleregisterunitbill(offpkunit, pkunit) {
  let offpkenergycost = offpkunit * lt_c2["offpkrate"];
  let pkenergycost = pkunit * lt_c2["pkrate"];
...
function singleregisterunitbill(srunit) {
  let srenergycost = srunit * lt_c2["srrate"];
